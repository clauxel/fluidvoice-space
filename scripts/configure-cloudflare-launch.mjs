...
const workerName = 'fluidvoice-space'
...
async function requestJson(url, options, provider) {
  ...
  if (!response.ok || data.success === false) {
    const errors = Array.isArray(data.errors)
      ...
      : text
    throw new Error(`${provider} request failed (${response.status}): ${errors}`)
  }
  return data
}
...
async function updateRegistrarNameservers(zone) {
  if (!spaceshipKey || !spaceshipSecret) {
    return { action: 'skipped', reason: 'Spaceship credentials not configured in environment' }
  }
...
async function status(zone, created) {
  ...
  const [dnsRows, routes, ssl, alwaysHttps, automaticHttpsRewrites] = await Promise.all([
